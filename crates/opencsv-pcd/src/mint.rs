...
//! 5. the prover knows the issuer seed committed by the asset genesis, and
//!    that genesis derives the public `asset_id` exactly as
//!    [`opencsv_core::AssetGenesis::asset_id`] does.
//!
...
use opencsv_core::{
    mint_commit, AssetGenesis, AssetId, Coin, Commitment, Digest, PoseidonIssuerAuthorization,
};
...
/// Number of private witness elements: mint_nonce (8), issuer authorization
/// (23), plus per output v (3) + owner (8) + r (8).
pub const MINT_PRIVATE_ELEMS: usize =
    DIGEST_ELEMS + ISSUER_AUTH_ELEMS + MINT_OUTPUTS * (VALUE_LIMBS + 2 * DIGEST_ELEMS); // 69
...
/// Slice the private input vector into the mint nonce, issuer authorization,
/// and per-output `(v, owner, r)` parts, in allocation order.
...
fn build_circuit() -> Result<Circuit<EF>, MintError> {
    ...
    // (e) issuer seed -> issuer key -> genesis -> claimed asset id.
    enforce_authorization(&mut builder, asset_id, &witness.issuer)?;
...
/// Prove an issuer-authorized mint for `outputs` created under `genesis`.
///
...
/// the module docs, including the issuer proof of knowledge.
pub fn prove_mint(
    ...
    if !PoseidonIssuerAuthorization::controls(issuer_secret, &genesis.issuer_pk) {
        return Err(MintError::IssuerKeyMismatch);
    }
